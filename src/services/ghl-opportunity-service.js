...
async function updateOpportunityToWon(opportunityId, winReason, correlationId) {
  ...
  // Use /status endpoint (correct endpoint for status updates)
  const statusUrl = `https://services.leadconnectorhq.com/opportunities/${opportunityId}/status`;
  ...
  try {
    ...
    if (!response.ok) {
      logOpportunity('error', correlationId, {
        action: 'update_won_status_failed',
        opportunity_id: opportunityId,
        status_code: response.status,
        error: responseData
      });
    ...
    logOpportunity('info', correlationId, {
      action: 'update_won_status_success',
      opportunity_id: opportunityId,
      response: responseData
    });
    ...
    if (error.name === 'AbortError') {
      logOpportunity('error', correlationId, {
        action: 'update_won_opportunity_timeout',
        opportunity_id: opportunityId,
        error: 'Request timeout after 10 seconds'
      });
      return { success: false, error: 'Request timeout' };
      ...
      logOpportunity('error', correlationId, {
        action: 'update_won_opportunity_exception',
        opportunity_id: opportunityId,
        error: error.message
      });
      return { success: false, error: error.message };
...
/**
 * Handle marking all open opportunities as won for a customer contact
 * NON-BLOCKING orchestrator function - captures all errors, never throws
 *
 * @param {string} contactPhone - Phone number to search contact
 * @param {string} customerTag - Tag that triggered customer status (e.g., "Cliente")
 * @param {string} correlationId - Correlation ID for request tracking
 * @returns {Promise<Object>} Result object with success status
 */
async function handleCustomerOpportunities(contactPhone, customerTag, correlationId) {
  const startTime = Date.now();

  logOpportunity('info', correlationId, {
    action: 'customer_opportunities_started',
    phone: contactPhone,
    tag: customerTag
  });
  ...
  try {
    ...
    const updateResults = [];
    const winReason = customerTag; // Use customer tag as win reason
    ...
    logOpportunity('info', correlationId, {
      action: 'customer_opportunities_completed',
      contact_id: ghlContactId,
      total_updated: openOpportunities.length,
      success_count: successCount,
      failed_count: failedCount,
    ...
    logOpportunity('error', correlationId, {
      action: 'customer_opportunities_error',
      phone: contactPhone,
      error: error.message,
      stack: error.stack,
      duration_ms: duration
